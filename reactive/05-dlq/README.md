...
`@MongoDlqOptions` + `@RetryPolicy` combination, same custom DLQ
collection — the handler signals its failure via `Mono.error(...)` and
the REST endpoint is a WebFlux `Flux` controller.

## What you'll learn

- That `@DeadLetterQueue` and `@MongoDlqOptions` work identically in
  reactive mode — there's no `Mono`-aware variant
...
- `src/main/java/.../DlqHandler.java` — `@RetryPolicy` + `@DeadLetterQueue` + `@MongoDlqOptions`, `Mono<Void>` handler
